...
export async function POST(req) {
  try {
    ...
    const lock = await redis.set(`ci:${user.tok}:${today}`, 1, { nx: true, ex: 86400 });
    if (!lock) {
    ...
    const gap = last ? Math.round((new Date(today) - new Date(last)) / 864e5) : 0;
    ...
    if (thing && typeof thing === "string") {
      const cleanThing = thing.replace(/[^a-zA-Z0-9 .,!?'\-]/g, "").replace(/[\x00-\x1f]/g, "").trim().slice(0, 40);
